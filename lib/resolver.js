...
(function() {
  var base_path, config, log, nested_file_resolver, pairtree, pairtree_file_resolver, path, resolve_image_path, resolver_base_path, sanitize_id, simple_file_resolver;
  ...
  pairtree_file_resolver = function(id, filename) {
    var pair_path;
    pair_path = pairtree.path(id);
    return path.join(base_path, pair_path, filename);
  };
  ...
  resolve_image_path = function(id) {
    ...
    image_path = config.get('resolver.type') === 'simple' ? simple_file_resolver(clean_id, filename) : config.get('resolver.type') === 'nested' ? nested_file_resolver(clean_id, filename) : config.get('resolver.type') === 'pairtree' ? pairtree_file_resolver(clean_id, filename) : void 0;
    return image_path;
  };

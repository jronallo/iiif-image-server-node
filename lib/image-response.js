...
(function() {
  var Parser, Validator, config, fs, iiif, image_cache, image_extraction, image_response, info_cache, log, path_for_image_cache_file, resolve_image_path, ttl;
      ...
      if (!err) {
        ...
        return fs.stat(image_path, function(err, stats) {
          ...
          if (err) {
            ...
            valid_request = image_info ? (validator = new Validator(params, image_info), validity = validator.valid(), validity ? log.info({
              valid: true,
              test: 'info',
              url: url,
              ip: req.ip
